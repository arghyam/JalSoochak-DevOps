...
CREATE TABLE common_schema.admin_user_token_table (
    ...
    CONSTRAINT chk_aut_token_type CHECK (
        token_type IN ('INVITE', 'RESET')
    ),
    CONSTRAINT chk_aut_metadata CHECK (
        (
            token_type = 'INVITE'
    ...
    CONSTRAINT fk_aut_deleted_by FOREIGN KEY (deleted_by) REFERENCES common_schema.tenant_admin_user_master_table (id),
    CONSTRAINT fk_aut_created_by FOREIGN KEY (created_by) REFERENCES common_schema.tenant_admin_user_master_table (id)
);
...
CREATE UNIQUE INDEX uq_active_admin_token ON common_schema.admin_user_token_table (email, token_type)
WHERE
    used_at IS NULL
    AND deleted_at IS NULL;

CREATE UNIQUE INDEX idx_admin_token_hash ON common_schema.admin_user_token_table (token_hash);

CREATE INDEX idx_admin_token_email ON common_schema.admin_user_token_table (email);